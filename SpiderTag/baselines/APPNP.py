...
from torch_geometric.nn import  global_mean_pool, APPNP
...
class FC(pl.LightningModule):
    ...
    def __init__(self, graph ,matrix, alpha , eps, k, a):
        super(FC, self).__init__()
        self.conv1 = APPNP(K=k , alpha= a)
    ...
    def forward(self, x, stage, negative_sample):
        ...
        data = self.graph
        nodes = self.conv1(data.x, data.edge_index)
...
model = FC(graph, np_array1, A_ALPHA, A_EPS, 3, 0.2)
